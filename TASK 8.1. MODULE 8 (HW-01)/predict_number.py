...
def predict_number(number: int = 1) -> int:
    ...
    while True:
        ...
        if intermediate_number > number:
            ...
            random_number = np.random.randint(beginning,
                                              end)  # предполагаемое число в новой конечной границе при условии, что искомое число меньше предпалагаемого
        elif intermediate_number < number:
            beginning = intermediate_number
            random_number = np.random.randint(beginning,
                                              end)  # предполагаемое число новой начальной границе при условии, что искомое число больше предпалагаемого
        else:
